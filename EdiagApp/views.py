...
def adm_bookedTets(request):
    slotid = request.GET.get("sid")
    data = Bookings.objects.filter(slot__id = slotid)
    return render(request,'adm_bookedTets.html',{"data":data})
...
def lab_bookedTets(request):
    userid = request.session["uid"]
    slotid = request.GET.get("sid")
    data = Bookings.objects.filter(slot__id = slotid)
    return render(request,'lab_bookedTets.html',{"data":data})
...
def user_bookslot(request):
    ...
    if request.POST:
        for c in data:
            if str(c.test) in request.POST:
                test = Tests.objects.get(test = str(c.test))
                tobooking = Bookings.objects.create(slot = slot,
                                                     test = test)
                tobooking.save()
        slot.user = user
        slot.save()
...
def user_pay(request):
    ...
    slotid = request.GET.get("sid")
    slot = Slots.objects.get(id = slotid)
    tests = Bookings.objects.filter(slot__id = slot.id)
    amount = 0
    for i in tests:
        amount = amount + i.test.price
    if request.POST:
...
def user_bookedTets(request):
    userid = request.session["uid"]
    slotid = request.GET.get("sid")
    data = Bookings.objects.filter(slot__id = slotid)
    return render(request,'user_bookedTets.html',{"data":data})
...
def user_predict(request):
    ...
    if request.POST:
        ...
        import pandas as pd
        import pickle
        with open('C:/Ediago/Ediagnostic/EdiagApp/randomforest_classifier.pkl', 'rb') as f:
            model = pickle.load(f)
        X_new = [[age,sex,cp,rbp,chol,fbs,ecg,hr,exang,oldpeak,slope,ca,thal]]
        ...
        ins = Prediction.objects.create(age=age,sex=sex,cp=cp,rbp=rbp,chol=chol,fbs=fbs,ecg=ecg,hr=hr,exang=exang,oldpeak=oldpeak,slope=slope,ca=ca,thal=thal,user=user,result=result)
        ins.save()
        id = ins.id
        return redirect(f"/print_res?id={id}")
...
def print_res(request):
    ...
    return render(request, "print_res.html", {"data": data})

def user_history(request):
    userid = request.session["uid"]
    data = Prediction.objects.filter(user=userid).order_by("-id")
